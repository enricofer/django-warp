...
def remove_dataset (request, dataset):
    ...
    if dataset_name == "__TRASH":
        empty_trash()
        message = _("TRASH empty")
        className = "alert-success"
    else:
        if georefs:
            message = _("Can't remove not empty Dataset %s" % dataset_name)
            className = "alert-warning"

        else:
            message = _("Dataset %s successfully removed" % dataset_name)
            className = "alert-success"
            dataset_obj.delete()
...
def georef_start(request, datasetId = None, idx = None):
    ...
    if output and output == 'json':
        response = JsonResponse({"id": source["id"]})
        return response
    else:
